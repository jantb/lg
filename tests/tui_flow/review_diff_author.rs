...
#[test]
fn review_mouse_wheel_stays_in_review_mode_even_over_left_panes() {
    ...
    app.state.review = Some(AssistedReview {
        ...
        nodes: vec![ReviewNode {
            ...
            depth: 0,
            title: "Summary".into(),
            body: (0..80).map(|idx| format!("line {idx}")).collect(),
            context: Vec::new(),
        }],
    });
    app.state.diff_offset = 18;
    ...
    assert_eq!(app.state.diff_source, lg::state::DiffSource::Review);
    assert_eq!(app.state.focus, Pane::Main);
    assert_eq!(app.state.diff_offset, 0);
}
...
fn review_panel_renders_and_copies_generated_pr_text() {
    ...
    app.state.review = Some(AssistedReview {
        ...
        nodes: vec![
            ReviewNode {
                ...
                body: vec!["- Run the changed tests.".into()],
                context: Vec::new(),
            },
            ReviewNode {
                id: lg::git::REVIEW_PR_TEXT_NODE_ID.into(),
    ...
    let rendered = buffer_text(&app);
    assert!(
        rendered.contains("PR text - generated by LLM"),
        "{rendered}"
    );
