---
name: terrarium-swift-tools
description: >
  Use when you need to run built-in terrarium MCP commands in Swift projects initialized
  with `terrarium`. The terrarium MCP tools run Swift build commands inside a strict macOS sandbox
  and return compact structured results.
  TRIGGER: terrarium-managed Swift project + any of: compile errors, run tests, check lints, check formatting.
---

{version_tag}

When working in a terrarium-managed Swift project, prefer the built-in MCP tools over Bash:

| Goal | Use |
|------|-----|
| Build | `swift_build` (optional: `release=true`) |
| Tests | `swift_test` (optional: single test name or subset filter) |
| Formatting | `swift_format_check` |
...
Do not bypass the terrarium MCP tools with direct Swift or read-only git Bash commands in projects managed by terrarium.
